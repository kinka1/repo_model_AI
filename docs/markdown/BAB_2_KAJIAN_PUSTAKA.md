...
VGG-16 dan VGG-19 merupakan arsitektur CNN yang menggunakan susunan convolution layer berukuran kecil, terutama kernel 3x3, secara bertumpuk. Desain ini sederhana dan mudah dipahami karena pola peningkatan kedalaman jaringan dilakukan secara bertahap. Kelebihan VGG adalah struktur layer yang konsisten, sehingga sering digunakan sebagai baseline dalam eksperimen klasifikasi citra. Namun, VGG memiliki jumlah parameter yang besar sehingga membutuhkan sumber daya komputasi lebih tinggi dan berpotensi mengalami overfitting pada data set terbatas.

ResNet

ResNet50 dan ResNet101 menggunakan konsep residual connection atau skip connection. Mekanisme ini memungkinkan informasi dari layer awal diteruskan langsung ke layer yang lebih dalam, sehingga membantu mengurangi masalah vanishing gradient pada jaringan yang sangat dalam. Dalam klasifikasi citra medis, ResNet sering digunakan karena mampu mempelajari fitur kompleks tanpa kehilangan informasi penting dari layer sebelumnya. Perbedaan utama ResNet50 dan ResNet101 terletak pada jumlah layer, di mana ResNet101 lebih dalam dan memiliki kapasitas representasi yang lebih besar.

EfficientNet

EfficientNet-B0 dan EfficientNet-B3 menggunakan pendekatan compound scaling, yaitu peningkatan kedalaman, lebar, dan resolusi jaringan secara seimbang. Tujuan pendekatan ini adalah memperoleh akurasi tinggi dengan penggunaan parameter dan komputasi yang lebih efisien. EfficientNet-B0 merupakan varian dasar yang lebih ringan, sedangkan EfficientNet-B3 memiliki kapasitas lebih besar. Arsitektur ini relevan untuk sistem klasifikasi citra bakteri karena dapat menyeimbangkan kebutuhan akurasi dan efisiensi inferensi.

DenseNet

DenseNet121 menggunakan dense connection, yaitu setiap layer menerima fitur dari layer-layer sebelumnya. Mekanisme ini mendorong pemanfaatan kembali fitur (feature reuse) dan membantu aliran gradien selama pelatihan. DenseNet dapat efektif pada data citra karena fitur dari berbagai tingkat kedalaman jaringan tetap dapat dimanfaatkan oleh layer berikutnya. Namun, performanya tetap bergantung pada jumlah data, kualitas citra, dan strategi pelatihan yang digunakan.
