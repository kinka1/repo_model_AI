...
## Restrukturisasi BAB 3 dan BAB 4 Mengikuti docs/main.pdf
- BAB 3: Mengubah struktur menjadi format bernomor seperti template PDF: `3.1 Deskripsi Solusi`, `3.2 Desain Sistem Model`, `3.3 Desain Sistem`, `3.4 Rancangan Antarmuka`, dan `3.5 Rancangan Skenario Uji Coba`.
- BAB 3: Merapikan narasi sistem agar fokus pada implementasi aktual project, yaitu YOLO11 untuk deteksi objek, CNN PyTorch untuk klasifikasi, FastAPI untuk backend, PostgreSQL untuk basis data, dan HTML/CSS/JS statis untuk frontend.
- BAB 3: Mengubah tabel dan daftar rancangan menjadi Markdown table yang lebih rapi serta memperbaiki path gambar Simple CNN menjadi `../images/simple_cnn_architecture.png`.
- BAB 4: Mengubah struktur menjadi format bernomor seperti template PDF: `4.1 Parameter Eksperimen`, `4.2 Karakteristik Data`, `4.3 Waktu Uji Coba`, `4.4 Spesifikasi Peralatan Uji Coba`, `4.5 Hasil Eksperimen`, dan `4.6 Analisis Eksperimen`.
- BAB 4: Merapikan tabel mentah hasil konversi DOCX menjadi Markdown table, memindahkan seluruh skenario ke bagian hasil eksperimen, serta memindahkan analisis perbandingan, confusion matrix, metrik evaluasi, dan loss function ke bagian analisis eksperimen.
- BAB 4 - ResNet50: Menggunakan data riil dari `models/metrics_resnet50.json` pada epoch terbaik ke-37, yaitu accuracy 0,952846, macro F1 0,952837, precision Gram-positif 0,965701, recall Gram-positif 0,939045, TN=1.681, FP=58, FN=106, TP=1.633, train loss 0,078946, dan validation loss 0,160809.
- BAB 4 - ResNet101: Menggunakan data riil dari `models/metrics_resnet101.json` pada epoch terbaik ke-60, yaitu accuracy 0,951409, macro F1 0,951404, precision Gram-positif 0,960681, recall Gram-positif 0,941346, TN=1.672, FP=67, FN=102, TP=1.637, train loss 0,049594, dan validation loss 0,186730.

## Perbaikan Konsistensi BAB 5 (Prioritas 1)
- BAB 5 - Kesimpulan: Memperbaiki klaim model terbaik yang sebelumnya salah ("EfficientNet-B3 tertinggi 98,46%, rata-rata 89,69%"). Angka ini berasal dari tabel ringkasan Skenario 5 di DOCX yang bertentangan dengan hasil per-arsitektur. Disinkronkan dengan Tabel 4.18 di BAB 4: rata-rata akurasi Skenario 5 = 92,17%, VGG-19 tertinggi (95,62%), dan ResNet50 dipilih sebagai kandidat terbaik karena seimbang (akurasi 95,17%, ROC-AUC 0,9883, 24,5 juta parameter, inferensi 2,94 ms). EfficientNet-B3 pada BAB 4 sebenarnya hanya 86,70%.
- BAB 5 - Kesimpulan: Memperbaiki klaim "VGG-16 dan DenseNet121 konsisten rendah di seluruh skenario" yang bertentangan dengan Tabel 4.18 (VGG-16 mencapai 95,31% pada Skenario 5). Diubah menjadi: DenseNet121 cenderung rendah, sedangkan VGG-16 lemah pada from scratch namun menyamai ResNet setelah transfer learning + fine-tuning penuh.
