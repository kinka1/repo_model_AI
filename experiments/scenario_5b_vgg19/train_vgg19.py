...
This script implements transfer learning using VGG19 pre-trained on ImageNet
for binary classification of Gram-positive and Gram-negative bacteria.

Features:
- Transfer learning with VGG19 backbone (deeper than VGG16)
...
EXP_DIR = ROOT_DIR / 'experiments' / 'scenario_5b_vgg19'
...
print("=" * 70)
print("🔬 VGG19 Training for Gram Bacteria Classification")
print("=" * 70)
...
class GramVGG19Classifier(nn.Module):
    """
    VGG19-based classifier for Gram bacteria classification.
    
    Architecture:
    - Backbone: VGG19 pre-trained on ImageNet (16 conv layers + pooling)
    ...
    Note:
    - VGG19 has 3 more conv layers than VGG16 (deeper feature extraction)
    - Expected to capture more complex patterns but slower training
    """
    ...
    def __init__(self, num_classes: int = 2, dropout_p: float = 0.5):
        ...
        print("📥 Loading VGG19 pre-trained weights from ImageNet...")
        self.backbone = models.vgg19(weights='IMAGENET1K_V1')
        
        # Replace classifier head
        # VGG19 features output: 7×7×512 = 25,088 features (same as VGG16)
        in_features = 25088
        ...
        print("✅ VGG19 model initialized with custom classifier head")
...
# Initialize model
model = GramVGG19Classifier(num_classes=NUM_CLASSES, dropout_p=DROPOUT_P).to(DEVICE)
...
def plot_training_curves(training_log: List[Dict], save_path: Path):
    ...
    fig.suptitle('VGG19 - Training Progress', fontsize=16, fontweight='bold')
...
def plot_confusion_matrix(cm: np.ndarray, save_path: Path):
    ...
    plt.title('VGG19 - Confusion Matrix', fontsize=14, fontweight='bold', pad=15)
...
def plot_roc_curve(labels: List, probs: List, save_path: Path):
    ...
    plt.figure(figsize=(8, 6))
    plt.plot(fpr, tpr, 'b-', linewidth=2.5, label=f'VGG19 (AUC = {auc:.4f})')
    plt.plot([0, 1], [0, 1], 'r--', linewidth=2, label='Random Classifier (AUC = 0.5000)')
    
    plt.xlabel('False Positive Rate', fontsize=12)
    plt.ylabel('True Positive Rate', fontsize=12)
    plt.title('VGG19 - ROC Curve', fontsize=14, fontweight='bold', pad=15)
...
if __name__ == '__main__':
    ...
    with open(EXP_DIR / 'classification_report.txt', 'w', encoding='utf-8') as f:
        f.write("VGG19 - Gram Bacteria Classification\n")
        f.write("=" * 70 + "\n\n")
    ...
    metrics_summary = {
        'scenario': 'Scenario 5b: Transfer Learning (VGG19)',
        'model': 'VGG19',
        'augmentation': True,
    ...
    print("\n🚀 VGG19 training script finished successfully!")
    print("\n" + "=" * 70)
